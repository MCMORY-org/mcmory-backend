...
public class SurveyController {
	...
	@Operation(summary = "설문 표시 정보 조회 (#34, Start-01)",
			description = "인증 불필요임. `senderName`은 익명 표시명이고 지금은 더미 `멋쟁이사자` 고정임 — 발송자 실명을 토큰 소지자에게 주지 않음(ADR-001 익명 원칙). `answered`가 true면 이미 답한 설문이며, 재제출은 덮어쓰기라 차단하지 않음. 없는 토큰과 삭제된 친구를 갈라주지 않고 모두 `FRIEND404_1`로 반환함.")
	@ApiResponses({
	...
	@Operation(summary = "설문 답변 제출 (#35, Start-02)",
			description = """
					...
					함정 셋임.
					1. `bags`는 저장만 하고 추천 점수에 쓰지 않음 — 시드 상품에 가방 디자인 축이 없어 매칭 대상이 없음.
					2. `colors`와 `styles`가 둘 다 비면 `FRIEND400_4`임. 선택지 밖 값도 조용히 버리지 않고 같은 코드로 막음.
					3. 브라우저에서 나가는 비인증 POST라 `OriginCheckFilter`를 지남 — 설문 페이지 오리진이 `ALLOWED_ORIGINS`에 없으면 제출이 전부 `COMMON403`임.
