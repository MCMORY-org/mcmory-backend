...
@Tag(name = "인증",
		description = "회원가입·로그인·세션 확인·재발급·로그아웃을 다룸. 액세스 토큰과 리프레시 토큰은 모두 HttpOnly 쿠키(`mcmory_at`, `mcmory_rt`)로 전달됨. 프론트엔드는 토큰을 별도로 저장하거나 인증 헤더에 넣지 말고 요청에 credentials를 포함할 것. 실패는 `message`가 아니라 응답의 `code`로 분기할 것.")
@RestController
...
public class AuthController {
	...
	public record LoginRequest(@Schema(
			description = "전화번호임. **로그인 ID가 전화번호임** — 이메일이 아님. " + "`010` 시작 10에서 11자리 숫자이며 하이픈을 넣어 보내도 서버가 숫자만 남겨 대조함",
	...
	public record SignupRequest(
			...
			@Schema(description = "전화번호임. 로그인 ID로도 쓰임. `010` 시작 10에서 11자리이며 하이픈을 넣어 보내도 되지만 "
					+ "**숫자만 저장함**. 형식이 어긋나면 `MEMBER400_2`, 이미 가입된 번호면 `MEMBER409_1`임", example = "01012345678",
			...
			@Schema(description = "개인정보 수집과 이용 동의임. **필수 동의라 `true`가 아니면 `CONSENT400_1`**임", example = "true",
					requiredMode = Schema.RequiredMode.REQUIRED) Boolean privacyAgreed,
	...
	@Operation(summary = "회원가입", description = "인증 불필요임. 성공 시 쿠키 2종(`mcmory_at`, `mcmory_rt`)을 발급해 가입 즉시 로그인 상태가 됨.\n\n"
	...
	@Operation(summary = "로그인",
			description = "인증 불필요임. 전화번호가 로그인 ID임 — 이메일이 아님.\n\n"
	...
	@Operation(summary = "세션 확인", description = "인증 선택임. 웹뷰에서 쿠키가 유지되는지 확인하는 지점임.\n\n"
	...
	@Operation(summary = "로그아웃",
			description = "인증 선택임. 쿠키 2종을 만료시키고 `refresh_token` 행을 삭제함.\n\n"
